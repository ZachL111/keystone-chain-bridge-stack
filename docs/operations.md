...
`keystone-chain-bridge-stack` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for bridge policy evaluation, using deny and allow fixtures and explainable decision traces.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 2.
